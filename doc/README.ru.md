...
- Пропуск, перемотка и уменьшение громкости неголосовых фрагментов в реальном времени
...
- [Для Windows (x64)](https://github.com/FilippVolodin/ReeePlayer/releases/download/v0.4/ReeePlayer-0.4-win-x64-installer.exe)
- [Для Windows (старая версия 0.1, x86)](https://github.com/FilippVolodin/ReeePlayer/releases/download/v0.1/ReeePlayer-0.1-win-x86.zip)
...
### Выбор движка видеоплеера

ReeePlayer поддерживает 2 движка: VLC и Web (Chromium). VLC поддерживает больше форматов и быстрее перематывает, но Web плавнее изменяет скорость воспроизведения. Попробуйте оба.

![](video_engine.png?raw=true)
...
- `5`, `6`, `7`, `8`, `9`, `0`, `-`, `=`, `Backspace` – скорость воспроизведения: 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.25, 1.5, 2.0 от нормальной
- `1`, `2` – показать текущий текст первых/вторых субтитров, если он скрыт
- `Enter` – добавить клип

Пропускать или ускореннно перематывать неголосовые фрагменты можно нажав кнопку ![](btn_jumpcutter.png?raw=true) на панели задач. Рекомендуется настроить обнаружение голосовых фрагментов нажатием на ![](jumpcutter_settings.png?raw=true).

 ![](jumpcutter_window.png?raw=true)

 ![](VAD_diagram.png?raw=true)

К сожалению, плееру требуется время (300-500 мс), чтобы изменить скорость после быстрой перемотки. Чтобы устранить негативный эффект, увеличьте значение в поле `Margin before`.
...
- Voice Activity Detector основан на модели [silero-vad](https://github.com/snakers4/silero-vad)
- [yt-dlp](https://github.com/yt-dlp/yt-dlp) для скачивания видео
- [ffmpeg](https://www.ffmpeg.org/) для генерации wav-файлов
- [Библиотека](https://github.com/adamstark/AudioFile) для чтения wav-файлов
